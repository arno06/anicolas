 ...
 /**
  ...
  private function AddAnAddress($kind, $address, $name = '') {
    if (!preg_match('/^(to|cc|bcc|ReplyTo)$/', $kind)) {
      trigger_error('Invalid recipient array: ' . $kind, E_USER_WARNING);
      return false;
    }
    ...
    if (!self::ValidateAddress($address)) {
     ...
     trigger_error($this->Lang('invalid_address').': '.$address, E_USER_ERROR);
      return false;
    }
    if ($kind != 'ReplyTo') {
    ...
    if(isset($images[2])) {
      foreach($images[2] as $i => $url) {
        ...
        if (!preg_match('#^[A-z]+://#',$url))
          ...
          if ( $this->AddEmbeddedImage($url, md5($filename), $filename, 'base64',$mimeType) ) 
          {
