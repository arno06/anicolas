 ...
 * Copyright (C) 2011  NICOLAS Arnaud <arno06@gmail.com> - http://www.minuit4.fr/
 *
...
if(typeof(M4)=="undefined") var M4 ={};

if(typeof(M4.browser)=="undefined")
{
	M4.browser = (function()
		{
...
if(typeof(M4.copy)=="undefined")
{
	M4.copy = function(t, c)
	{
		for(var i in c)
			t[i] = c[i];
	};
...
M4Tween.prototype =
{
	...
	start:function()
	{
		M4Tween.killTweensOf(this.target, true);
		...
		if(M4Tween.first)
		{
			M4Tween.first.prev = this;
			this.next = M4Tween.first;
		}
		if(typeof(this.startHandler)=="function")
			this.startHandler();
		M4Tween.first = this;
		if(!M4Tween.frameHandler)
			M4Tween.frameHandler = setInterval(M4Tween.step, M4Tween.TIME);
	},
	update:function(pDt)
	{
	...
	kill:function(pTrigger)
	{
		...
		M4Tween.available++;
		if(typeof(pTrigger)=="boolean"&&pTrigger&&typeof(this.completeHandler) == "function")
...
M4Tween.to = function(pTarget, pTime, pProperty)
{
	if(!pTarget)
		return;
	var a, ease = Quad.easeInOut,firstInfos,tmp, property, style, delay = 0;
	...
	for(var i in pProperty)
	{
		...
		if(typeof(M4TweenPlugins[property])!="object")
			continue;
		a = M4TweenPlugins[property].newInfos(i, pProperty[i], style);
	...
	if(delay)
		instance.waiting = setTimeout(function(){instance.start.apply(instance);}, delay * 1000);
	else
		instance.start();
	return instance;
};
...
M4Tween.step = function()
{
	...
	if(!i)
	{
		clearInterval(M4Tween.frameHandler);
		M4Tween.frameHandler = null;
		return;
	}
...
M4.copy(M4TweenPlugins.backgroundColor, M4TweenPlugins.color);
...
M4TweenPlugins.rotate =
{
	...
	newInfos:function(pProperty, pFinalValue)
	{
		...
		if(M4.browser.IE)
			...
			p = "MozTransform";
		var i = new M4TweenInfos(p, s, t, tpl);
		return i;
	}
};
