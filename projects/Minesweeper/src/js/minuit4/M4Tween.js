...
M4Tween.prototype =
{
	configure:function (pTarget, pFirstInfos, pDuration, pEase, pDelay, pStyle)
	{
		...
		this.onStart(null).onUpdate(null).onComplete(null);
		this.startTime = null;
		this.delay = pDelay;
		this.target = pTarget,
	...
	start:function()
	{
		...
		M4Tween.first = this;
		if(!M4Tween.running)
			M4Tween.running = window.setTimeout(M4Tween.step, M4Tween.TIME);
	},
	update:function(pDt)
	{
...
M4Tween.step = function()
{
	...
	if(!i)
	{
		window.clearTimeout(M4Tween.running);
		M4Tween.running = null;
		return;
	}
